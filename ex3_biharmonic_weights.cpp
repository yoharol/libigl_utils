...
int main(int argc, char *argv[]) {
  ...
  std::cout << "vertices: " << V.rows() << " " << V.cols() << std::endl;
  std::cout << "faces: " << F.rows() << " " << F.cols() << std::endl;
  U = V;
  ...
  b.conservativeResize(
      stable_partition(b.data(), b.data() + b.size(),
                       [&S](int i) -> bool { return S(i) >= 0; }) -
      b.data());
  // b: indices of vertices in handle
  ...
  // If all 3 vertices of a face is handeled, make it purple
  // Gold otherwise
  MatrixXd C(F.rows(), 3);
  ...
  viewer.core().trackball_angle =
      Eigen::Quaternionf(sqrt(2.0), 0, sqrt(2.0), 0);
  ...
  cout << "Press [space] to toggle deformation." << endl
       << "Press 'd' to toggle between biharmonic surface or displacements."
       << endl;
  viewer.launch();
}