...
bool pre_draw(igl::opengl::glfw::Viewer &viewer) {
  ...
  for (int i = 0; i < b.size(); i++) {
    ...
    if (i == 0) {
      ...
      double period = 0.30;
      double axis[2] = {0.0, 0.2};
      bc(i, 0) = bc_ref(i, 0) + axis[0] * sin(period * signal);
      bc(i, 1) = bc_ref(i, 1) + axis[1] * sin(period * signal);
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  auto stage = pxr::UsdStage::CreateNew(assets_path + cnslash + "arap.usda");
