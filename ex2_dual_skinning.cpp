...
int main(int argc, char *argv[]) {
  ...
  igl::direct_delta_mush_precomputation(V, F, W, p, lambda, kappa, alpha,
                                        Omega);
  ...
  Eigen::RowVector3d offset(1.1 * (V.col(0).maxCoeff() - V.col(0).minCoeff()),
                            0, 0);

  viewer.callback_pre_draw = [&](igl::opengl::glfw::Viewer &) -> bool {
    if (viewer.core().is_animating) {
      const Eigen::Map<Eigen::MatrixXd> Tf(T.data() + frame * T.rows(),
                                           4 * W.cols(), 3);
...

      {
        ...
        for (int e = 0; e < BE.rows(); e++) {
          ...
          T_list[e].matrix().block(0, 0, 3, 4) =
              Tf.block(e * 4, 0, 4, 3).transpose();
        }
  ...
  for (auto &id : {pr_id, ddm_id}) {
    if (id == pr_id) {
      ...
      viewer.data(id).set_colors(
          Eigen::RowVector3d(214. / 255., 170. / 255., 148. / 255.));
      viewer.data(id).set_edges(C, BE, Eigen::RowVector3d(1, 1, 1));
    } else if (id == ddm_id) {
      viewer.data(id).set_mesh((V.rowwise() + offset * 1.0).eval(), F);
      viewer.data(id).set_colors(
          Eigen::RowVector3d(132. / 255., 214. / 255., 105. / 255.));
    }
