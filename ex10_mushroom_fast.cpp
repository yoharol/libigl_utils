...
std::string assets_name = "mushroom";
std::vector<int> fix_points({0, 1});
...
bool pre_draw(igl::opengl::glfw::Viewer &viewer) {
  ...
  if (resolve) {
    ...
    VectorXd Beq(3 * (fix_points.size() + 3));
    ...
    for (int i = 0; i < fix_points.size(); i++) {
      ...
      switch (idx) {
        case 1:
          ...
          double x = signal * 1.2;
          double y = (0.6 - x * x) * 0.3;
          bc(idx, 0) = bc_ref(idx, 0) + x * 0.11;
          bc(idx, 1) = bc_ref(idx, 1) + y * 0.11;
          break;
      }
    }
    for (int tmp = 0; tmp < 4; tmp++) {
      ...
      Beq(3 * tmp + 1) = 0.0;
      Beq(3 * tmp + 2) = 0.0;
      if (tmp < 3) Beq(3 * tmp + tmp) = 1.0;
    }
    for (int i = 1; i < fix_points.size(); i++) {
      int idx = fix_points[i];
      Beq(3 * (i + 3) + 0) = bc(idx, 0);
      Beq(3 * (i + 3) + 1) = bc(idx, 1);
      Beq(3 * (i + 3) + 2) = bc(idx, 2);
    }
    {
      VectorXd L0 = L;
...
int main(int argc, char *argv[]) {
  ...
  auto stage = pxr::UsdStage::CreateNew(assets_path + cnslash + "fast.usdc");
  ...
  // number of weights
  const int m = W.cols();
  Aeq.resize((fix_points.size() + 3) * 3, m * 3 * (3 + 1));
  // Aeq.resize(m * 3, m * 3 * (3 + 1));
  vector<Triplet<double> > ijv;

  for (int tmp = 0; tmp < 4; tmp++) {
    int idx = fix_points[0];
    Vector4d v;
    v(0) = 0.0;
    v(1) = 0.0;
    v(2) = 0.0;
    ...
    for (int d = 0; d < 3; d++) {
      for (int c = 0; c < (3 + 1); c++) {
        ijv.push_back(
            Triplet<double>(3 * tmp + d, idx + c * m * 3 + d * m, homo(c)));
      }
    }
  }
  ...
  for (int i = 1; i < fix_points.size(); i++) {
    ...
    for (int d = 0; d < 3; d++) {
      for (int c = 0; c < (3 + 1); c++) {
        ijv.push_back(
            Triplet<double>(3 * (i + 3) + d, idx + c * m * 3 + d * m, homo(c)));
      }
    }
  }
